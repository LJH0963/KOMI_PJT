...
- **LLM 통합 기능**:
  - Ollama 기반 LLM 모델 로드 및 활용
  - 포즈 분석 결과에 기반한 맞춤형 프롬프트 생성
  - 의료/재활 지식 활용한 전문적 피드백 제공
- **포즈 분석 알고리즘**:
  - 관절 각도 계산 및 분석
  - 벡터 기반 코사인 유사도 측정
  - 기준 포즈와 사용자 포즈 간 정확도 점수화
- **비디오 처리 파이프라인**:
  - 업로드된 영상에서 분석용 프레임 추출
  - YOLO 모델 기반 포즈 인식 및 키포인트 추출
  - 동영상 업로드 및 분석 전체 프로세스 관리
- **실시간 통신**:
  - WebSocket 기반 카메라 스트림 중계
  - 다중 클라이언트 연결 관리 및 브로드캐스트
  - 안정적인 연결 유지 및 중단 복구 메커니즘

### 웹캠 클라이언트 (webcam_client.py)
- **카메라 작동 모드**:
  - `mask` 모드: 자세 인식을 위한 가이드 마스크 오버레이
  - `record` 모드: 분석용 영상 녹화 및 서버 업로드
  - `detect` 모드: 실시간 자세 감지 및 문제점 시각화
- **자세 분석 기능**:
  - 실시간 포즈 유사도 계산 및 정확도 평가
  - 키포인트 기반 자세 문제점 분석
  - 각도 기반 자세 정렬 확인
- **UI 개선**:
  - 한글 텍스트 지원 (운동 가이드 및 피드백)
  - 오버레이 효과 (마스크, 카운트다운, 분석 결과)
  - 사용자 친화적 시각적 피드백

### Streamlit 앱 (streamlit_app.py)
- **페이지 구조**:
  - 운동 선택: 다양한 재활 운동 옵션 제공
  - 운동 가이드: 전/측면 동기화 가이드 영상 제공
  - 자세 분석: 실시간 카메라 스트리밍 및 마스크 모드
  - 분석 결과: 자세 유사도 점수 및 LLM 기반 피드백
  - 실시간 피드백: 운동 중 즉각적인 자세 교정 정보
- **다중 카메라 동기화**:
  - 타임스탬프 기반 프레임 동기화 알고리즘
  - 서버-클라이언트 시간 동기화 메커니즘
  - 최대 허용 시간차 설정을 통한 정확한 동기화
- **비동기 UI 처리**:
  - 백그라운드 이미지 처리 및 UI 업데이트
  - 스레드 풀 활용한 효율적인 리소스 관리
  - WebSocket 연결 자동 복구 및 재시도 메커니즘
...
| `/exercises` | GET | 운동 목록 조회 |
| `/exercise/{exercise_id}` | GET | 특정 운동 상세 정보 조회 |
| `/videos/upload` | POST | 동영상 업로드 |
| `/videos/{video_id}/analyze` | POST | 업로드된 동영상 분석 요청 |
| `/analysis/video/{video_id}` | GET | 동영상 분석 결과 조회 |
| `/cameras/{camera_id}/status` | POST | 카메라 상태 변경 (mask/record/detect/off) |
...
- **주요 기능**:
  ...
  - 포즈 감지 결과 시각화 및 운동 가이드 표시
  - LLM 기반 의료 상담 및 운동 추천 UI
...
### 연결 안정성 개선
- 연결 끊김 자동 복구 및 지수 백오프 재시도 구현
- 핑/퐁 메커니즘을 통한 연결 상태 모니터링
- 타임아웃 처리 및 예외 상황 대응 로직 강화

### 성능 최적화
- 이미지 압축 및 크기 조절을 통한 대역폭 사용 최적화
- 비동기 처리 및 스레드 풀을 통한 UI 응답성 향상
- 클라이언트-서버 간 효율적인 데이터 전송 프로토콜

### 동기화 시스템 개선
- 타임스탬프 기반 프레임 동기화 알고리즘 구현
- 서버 시간 기준 클라이언트 시간 조정 메커니즘
- 최대 허용 시간차(MAX_SYNC_DIFF_MS) 설정 최적화
...
- YOLO11 기반 포즈 감지 모듈 고도화 및 경량화
- 시간적 흐름을 고려한 Dynamic Time Warping 구현
- 다양한 환경(조명, 거리, 의상)에 대한 인식률 향상
- 실시간 분석 중 중요 문제점 음성 알림 기능
- 모바일 호환성 향상 및 반응형 UI 개선
- 사용자별 운동 기록 저장 및 진행 추적 기능
